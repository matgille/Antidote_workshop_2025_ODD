...
\title[TEI schemata]{Introduction to TEI schemata\\ {\small Antidote workshop, Klosterneuburg}}
...
\item As said before, you'll need to create and write the documentation \textit{in natural language}, using the TEI.
\item You can use any XML editor to do so.
\end{itemize}
\begin{figure}
\includegraphics[height=.25\textheight]{img/example_ODD.png}
...
\begin{minted}{xml}
 <sch:pattern>
      <sch:title>Foliation checks</sch:title>
      <sch:rule context="pb">
         <sch:assert
            test="matches(@n, '\d+[vr]')"
            >Something is wrong with the foliation of this page beggining. Have you forgotten the page side indication ?</sch:assert>
      </sch:rule>
   </sch:pattern>
\end{minted}
\end{frame}
